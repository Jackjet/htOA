...
public class Helper {
	...
	public static String joinAddress(Address[] a) {
		...
		s = MessageParserCommon.dealUnkownEncoding(s);
	...
	public static String encryptTEA(String src) {
		...
		for (int i = 0; i < enc.length; i++) {
			s += Integer.toHexString((byte) (enc[i] >>> 24) + 128)
					.toUpperCase()
					+ ":";
			s += Integer.toHexString((byte) (enc[i] >>> 16) + 128)
					.toUpperCase()
					+ ":";
	...
	/**
	 * Here we figure out which quote level this line has, simply by counting
	 * how many ">" are in front of the line, ignoring all whitespaces.
	 */
	public static int getQuoteLevel(String token) {
		int current_quotelevel = 0;
		for (int i = 0; i < token.length()
				&& (token.substring(i).startsWith(">") || token.substring(i)
						.startsWith(" ")); i++) {
