...
public class JavaScriptCleaner {
	...
	protected void walkTree(Node node) {
		...
		if (node instanceof Element
			...
			((Element) node).setAttribute("malicious",
		...
		if (node instanceof Element
			...
			((Element) node).setAttribute("malicious",
					"Marked malicious because of potential Image/CGI abuse");
		}
		...
		/*
		 * Now we search the attribute list for attributes that may potentially
		 * be used maliciously. These will be: - href: check for a String
		 * containing "javascript" - onXXX events: if they exist, the link will
		 * be marked "malicious".
		 */
		...
		if(map!=null){
			for (int i = 0; i < map.getLength(); i++) {
				...
				if (a.getName().toUpperCase().equals("HREF")) {
					for (int j = 0; j < a.getValue().length()
							- javascript_href.length(); j++) {
					...
					/*
					 * All elements containing "onXXX" tags get the malicious
