...
public class ExpireableCache extends Thread {
	...
	/**
	 * Implement a simple heap that just returns the smallest long
	 * variable/Object key pair.
	 */
	protected class MyHeap {
		...
		/**
		 * Return and delete the key with the lowest long value. Reorganize
		 * Heap.
		 */
		public Object next() {
			Object ret = keys[0];
		...
		/**
		 * Remove an Object from the Heap. Unfortunately not (yet) of very good
		 * complexity since we are doing a simple linear search here.
		 * 
		 * @param key
		 *            The key to remove from the heap
		 */
		...
		/**
		 * Lift an element in the heap structure Note that the cur_pos is
		 * actually one larger than the position in the array!
		 */
		...
		/**
		 * Lower an element in the heap structure Note that the cur_pos is
		 * actually one larger than the position in the array!
		 */
		protected void decrease(int cur_pos) {
			while ((cur_pos * 2 <= num_entries && values[cur_pos - 1] > values[cur_pos * 2 - 1])
				...
				if (cur_pos * 2 + 1 <= num_entries) {
					lesser_son = values[cur_pos * 2 - 1] < values[cur_pos * 2] ? cur_pos * 2
							: cur_pos * 2 + 1;
				} else {
