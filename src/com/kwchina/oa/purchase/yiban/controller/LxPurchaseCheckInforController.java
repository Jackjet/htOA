...
@Controller
@RequestMapping("/lxpurchase/checkInfor.do")
public class LxPurchaseCheckInforController extends PurchaseBaseController {
...
//				SystemUserInfor systemUser = SysCommonMethod.getSystemUser(request);
				...
				if (systemUser.getPerson().getPersonId() == caigoulingdao) {
					...
					this.purchaseManager.save(purchase);
					purchaseId =purchase.getPurchaseId();
				}else if (systemUser.getPersonId() == organizeInfor.getDirector().getPersonId()){
					int pstatus = 1;
					purchase.setPurchaseStatus(pstatus);
					this.purchaseManager.save(purchase);
					purchaseId =purchase.getPurchaseId();
