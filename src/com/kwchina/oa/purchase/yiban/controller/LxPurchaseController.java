...
@Controller
@RequestMapping("/lxpurchase/purchaseInfor.do")
public class LxPurchaseController extends PurchaseBaseController {
...
//					status = "财务预算审核中";
				...
				if ( fi.getPurchaseStatus() == PurchaseInfor.Purchase_Status_No
						||  fi.getPurchaseStatus() == 9 ||  fi.getPurchaseStatus() == 10){
					canCheck = "false";
				}
...
//		purchase.setSubmiterWord(null);
		...
		int batchNumber = 1;
		String sql2 ="select max(a.batchNumber) from PurchaseInfor a where 1=1 and a.deleteFlag = 0 and a.flowId.flowId = 4 ";
	...
	public void getNeedDealInstances(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(systemUser != null){
			...
			if(isMobile){
				...
				//得到待办
				Map<String, Object> map = this.purchaseManager.getNeedDealInstances(systemUser,4);
			...
			if (updateIds.length > 0) {
				for (int j = 0; j < updateIds.length; j++) {
					...
					if (layer.getStatus() == 1) {
						...
						String hql = "from PurchaseNode node where flowId = 4 and nodeId =" + newlayerNode ;
...
//							purchasePackage.setPackageName(strn +"   采购计划");
						...
						purchasePackage.setStatus(0);
						purchasePackage.setCheckerType(0);
						purchasePackage.setManager(systemUser.getPerson().getDepartment().getDirector().getUser());
						purchasePackage.setFlowId(4);
						purchasePackage.setPurchaseInfors(purchaseInfors);
						purchasePackage.setStartDate(currentTime);
//						String strn = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(currentTime);
							...
							for (Iterator itCheck = checks.iterator(); itCheck.hasNext(); ) {
								...
								if (check.getChecker() != null) {
									if (check.getStatus() == 0 && check.getChecker().getPersonId() == personId) {
										...
										if (hasAllchecked) {
											int pstatus = purchase.getPurchaseStatus();
											if (purchase.getPurchaseWay() == 0 && purchase.getPurchaseStatus() == 7) {
												pstatus = pstatus + 2;
											} else {
												pstatus = pstatus + 1;
											}
			...
			if (rowIds != null && rowIds.length() > 0) {
				...
				if (updateIds.length > 0) {
					for (int j = 0; j < updateIds.length; j++) {
						...
						if (purchase.getPurchaseStatus() == 6  ){
							candisagree = false;
						}
						if (candisagree) {
							List rLayers = this.purchaseManager.getCurrentProcessLayers(purchase);
		...
		if (rowIds != null &&rowIds.length() > 0) {
			...
			if (updateIds.length > 0) {
				for (int j = 0; j < updateIds.length; j++) {
					...
					if (purchase.getPurchaseStatus() == 0 ||purchase.getPurchaseStatus() == 7 ){
						canback = false;
					}
					if (canback) {
						List rLayers = this.purchaseManager.getCurrentProcessLayers(purchase);
