...
public class ClubSignBeginTask extends TimerTask { 
		 
	private ClubSignBeginTask(){}
	   
	private static ClubSignBeginTask _checkTask = new ClubSignBeginTask();
	public static ClubSignBeginTask getCheckLicensidTask(){
		return _checkTask;
	}
...
//				this._isRunning = true;
				...
				System.out.println("-------------------------开始执行活动自动开始报名："+new Timestamp(System.currentTimeMillis())+"-------------------------");
				
				/*
				 * 
				 * 首先得到所有未完成的、报名开始日期在今天的活动列表，自动将其状态改为报名中
				 * 
				 */
...
//				
				...
				for(ClubInfor club : unFinishedInfors){
					if(club.getStatus() == ClubInfor.Club_Status_ToReg && club.getBeginSignDate() != null){
						String beginSignTime = sf.format(club.getBeginSignDate());
//						if(beginSignTime.equals(todayStr)){
						if(beginSignTime.equals(todayStr) || DateHelper.getDate(beginSignTime).before(DateHelper.getDate(todayStr))){
							System.out.println("*****************操作了：" + club.getActTitle() + "*******************");
							clubInforManager.changeTaskStatus(club, ClubInfor.Club_Status_Reging);
						}
					}
...
//				this._isRunning = false;
				...
				System.out.println("-------------------------活动自动开始报名执行完成："+new Timestamp(System.currentTimeMillis())+"-------------------------");
//			}else{
...
//			}
			...
			e.printStackTrace();
			System.out.println("------------------------活动自动开始报名执行出错-------------------------");
//			this._isRunning = false;
		}
