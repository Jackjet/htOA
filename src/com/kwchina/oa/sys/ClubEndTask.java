...
public class ClubEndTask extends TimerTask { 
		 
	private ClubEndTask(){}
	   
	private static ClubEndTask _checkTask = new ClubEndTask();
	public static ClubEndTask getCheckLicensidTask(){
		return _checkTask;
	}
...
//				this._isRunning = true;
				...
				System.out.println("-------------------------开始执行自动结束活动："+new Timestamp(System.currentTimeMillis())+"-------------------------");
				
				/*
				 * 
				 * 首先得到所有未完成的、结束日期在今天的活动列表，自动将其状态改为结束
				 * 
				 */
...
//				
				...
				for(ClubInfor club : unFinishedInfors){
					if(club.getStatus() == ClubInfor.Club_Status_Acting && club.getToTime() != null){
						String toTime = sf.format(club.getToTime());
//						if(toTime.equals(todayStr)){
						if(toTime.equals(todayStr) || DateHelper.getDate(toTime).before(DateHelper.getDate(todayStr))){
							System.out.println("*****************操作了：" + club.getActTitle() + "*******************");
							clubInforManager.changeTaskStatus(club, ClubInfor.Club_Status_Over);
						}
					}
...
//				this._isRunning = false;
				...
				System.out.println("-------------------------自动结束活动执行完成："+new Timestamp(System.currentTimeMillis())+"-------------------------");
//			}else{
