...
<script language=javascript>
						...
						function Open_AddrWin(){
							var returnValue;
							returnValue = window.showModalDialog('<c:url value="${'/webmail/mailMessage.mdo'}"/>?method=getPerson','Select Person','dialogHeight:500px;dialogWidth:650px;dialogLeft:300');
							//window.open('<c:url value="${'/webmail/mailMessage.mdo'}"/>?method=getPerson');
							//alert(returnValue);
							//alert(returnValue!=null);
							
							var to = document.mailForm.to.value;
							var cc = document.mailForm.cc.value;
							var bcc = document.mailForm.bcc.value;
							//alert(to.length);
							
							if(returnValue!=null){
								var mails = returnValue.split("|");
								var ato = mails[0];
								var acc = mails[1];
								var abcc = mails[2];								
								...
								//to-----
								if(ato!=null && ato!=""){									
									if(to!="" && to.substring(to.trim().length-1,to.trim().length)!=","){
										to = to + ", " + ato; 
									}else{
										to = to + ato; 
									}
								...
								//cc--------
								if(acc!=null && acc!=""){									
									if(cc!="" && cc.substring(cc.trim().length-1,cc.trim().length)!=","){
										cc = cc + ", " + acc; 
									}else{
										cc = cc + acc; 
									}
								...
								//bcc-------------
								if(abcc!=null && abcc!=""){									
									if(bcc!="" && bcc.substring(bcc.trim().length-1,bcc.trim().length)!=","){
										bcc = bcc + ", " + abcc; 
									}else{
										bcc = bcc + abcc; 
									}
							...
							document.mailForm.cc.value =  cc;
							document.mailForm.bcc.value =  bcc;						
						}						
	</script>
...
jq().ready(function() {
	var emails = [
		<c:forEach items="${requestScope._Company_Persons}" var="cPerson" varStatus="status">			
			<c:choose>
				<c:when test="${status.index>0}">
					,{ name: "${cPerson.personName}", to: "${cPerson.email}" }
				</c:when>
				<c:otherwise>
					{ name: "${cPerson.personName}", to: "${cPerson.email}" }
				</c:otherwise>	
		...
		<c:forEach items="${requestScope._Self_Persons}" var="sPerson" varStatus="status">
			,{ name: "${sPerson.personName}", to: "${sPerson.email}" }			
		</c:forEach>		
	];
...
<DIV id="Main">
	<html:form method="post" action="/webmail/mailMessage.mdo" enctype="multipart/form-data" onsubmit="return chk();">
									...
									<TABLE width="100%" cellpadding="0" cellspacing="0">
											...
											<TD style="padding-top:5px;">
												<TABLE width="95%" cellpadding="0" cellspacing="2" id="tbMain">
													<TR id="TRto">
														...
														<a href="javascript:Open_AddrWin();">收件人</a>:
														</SPAN></TD>
														<TD class="altbg2"> 
															<input type="text" id="to" name="to" chkName="收件人" chkRule="notnull/emls" style="width:100%;height:25" class="ACtype" tabindex="1" onKeyDown="ctlent(event);" value="${_Work_Message.to}">												
													...
													<TR id="TRcc" bgcolor="#F6F6F6" style="display:none">
														...
														<TD>
															<input type="text" id="cc" name="cc" chkName="抄送" chkRule="emls" style="width:100%;height:25" class="ACtype" tabindex="4" onKeyDown="ctlent(event);" value="${_Work_Message.cc}">
													...
													<TR id="TRbcc" bgcolor="#F6F6F6" style="display:none">
														...
														<TD>
															<input type="text" ic="bcc" name="bcc" chkName="暗送" chkRule="emls" style="width:100%;height:25" class="ACtype" tabindex="5" onKeyDown="ctlent(event);" value="${_Work_Message.bcc}">
													...
													<TR style="height:28">
														...
														<TD class="altbg2">
															<INPUT type="text" name="subject" id="subject" style="width:100%" onKeyDown="ctlent(event);" tabindex="6"  value="${_Work_Message.subject}"/>
														</TD>
													</TR>
													<!-- 
													...
													<TR>
														...
														<TD id="TDAttach">
															<DIV width="200" style="border-bottom: 0px solid #aaa" id="DIVAttach">
																<UL id="ulAttach">
																	<c:forEach items="${requestScope._Work_Parts}" var="part" varStatus="status">					  			
																	  	<c:if test="${!part.hidden && (part.type=='binary' ||  part.type=='image')}">															  																			  			
																	  		<li><input type=checkbox checked name="REMOVE-${part.partCount}"> ${part.fileName} (${part.size})</li> 
																	  	</c:if>
																	</c:forEach>
																</UL>
														 	</DIV>
										...
										<TD class="TDmain_in_editor">
													...
													<TD align="left" class="altbg2" valign="top">										
														<DIV id="posteditor">
															...
															<DIV class="editor_text" style="width:100%;">
																<TEXTAREA name="message" rows="10" cols="60" style="width:99%; height:250px;font-size:14px;font-family:Arial;padding:5px;" id="posteditor_TEXTAREA" onSelect="javaScript: storeCaret(this);" onClick="javaScript: storeCaret(this);" onKeyUp="javaScript:storeCaret(this);" onKeyDown="ctlent(event);" tabindex="100" type="_moz">													
											...
											<SCRIPT language="javaScript">
												...
												//alert($('test').value);
												var fixContent = "";
												<logic:present name="_Work_Message">													
													fixContent = '----- Original Message ----- <br>';
													fixContent = fixContent + '<b>From</b>: ${_Old_Message.from}<br>';
													fixContent = fixContent + '<b>To</b>: ${_Old_Message.to}<br>';
													fixContent = fixContent + '<b>Sent</b>: ${_Old_Message.getDate}<br>';
													fixContent = fixContent + '<b>Subject</b>: ${_Old_Message.subject}';
													fixContent = fixContent + '<br><br>';																																
												</logic:present>
