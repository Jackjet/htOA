...
function newEditor(mode, initialtext) {
	...
	if(wysiwyg) {
		...
		var objValue = "<br>";
		if(fixContent!=""){			
			objValue = objValue + "<blockquote style='border-left: 2px solid rgb(204, 204, 204); margin: 0pt 0pt 0pt 0.8ex; padding-left: 1ex;'>";
			
			//alert(objValue.indexOf("<"));
			fixContent.replaceAll("<","&lt;");
			fixContent.replaceAll(">","&gt;");
			//alert("-----------------");
			//alert(objValue);
			objValue = objValue + fixContent + textobj.value + "</blockquote>";	
		}else{
			objValue = textobj.value;
		}
		
		//alert(objValue);
		writeEditorContents(isUndefined(initialtext) ? objValue : initialtext);
	} else {
		editbox = textobj;
		editwin = textobj;
		editdoc = textobj;
