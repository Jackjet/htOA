...
function ctlent(event) {
	if(postSubmited == false && (event.ctrlKey && event.keyCode == 13) || (event.altKey && event.keyCode == 83) && $('postsubmit')) {
		//if(in_array($('postsubmit').name, ['topicsubmit', 'replysubmit', 'editsubmit']) && !validate($('mailForm'))) {
		//	return;
		//}
		DoSend();
	}
...
function checkFocus() {
	var obj = typeof wysiwyg == 'undefined' || !wysiwyg ? $('mailForm').message : editwin;
	if(!obj.hasfocus) {
		obj.focus();
	}
}
