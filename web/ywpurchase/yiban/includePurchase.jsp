...
<script>
	...
	<%--function editCheckInfor(checkId) {--%>
		<%--var path = "<c:url value='/ywpurchase/checkInfor.do'/>?method=edit&chargerEdit=true&instanceId=${_Purchase.instanceId}&checkId="+checkId;--%>
		<%--window.name = "__self";--%>
...
<link href="<c:url value="/"/>css/submit.css" type="text/css" rel="stylesheet">
	...
	<c:if test="${Purchase.flowId.flowId == 1 }">
		...
		<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-btable" style="width: 90%">
			<tbody id="tblGrid">	
						<c:if test="${_Purchase.flowId.flowId == 1||_Purchase.flowId.flowId == 2}">
						...
						<c:if test="${_Purchase.purchaseGoods != null}">
							<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
								<td class="ui-state-default jqgrid-rownum" style="width: 20%">预算类型：</td>
						...
						<c:if test="${_Purchase.purchaseStr2 != null}">
							<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
								<td class="ui-state-default jqgrid-rownum" style="width: 20%">业务板块：</td>
								<td>${_Purchase.purchaseStr2}</td>
							</tr>
						</c:if>
						<c:if test="${_Purchase.purchaseStr1 != null}">
							<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
								<td class="ui-state-default jqgrid-rownum" style="width: 20%">采购类型：</td>
						...
						<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
							<td class="ui-state-default jqgrid-rownum" style="width: 20%">需求内容及原因：</td>
							<td>${_Purchase.guige}</td>
						</tr>
						<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
							<td class="ui-state-default jqgrid-rownum" style="width: 20%">运作方案及标准：</td>
