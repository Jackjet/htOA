	...
	<script type="text/javascript">     
		...
		function doView(rowId){
  		...
  		<div style="margin-top:0px;">
			<table id="listPlanActivity" style="width:99%"></table>
			<div id="pagerPlanActivity"></div>
		</div>	
		
		<!-- 查询框 -->
		<div id="multiSearchDialogPlanActivity" style="display: none;">  
		    <table>  
		        <tbody>  
		            <tr>  
		                <td>  
		...
		<script type="text/javascript"> 		
		   ...
		   function formatCompanyName(cellValue, options, rowObject) {			
			...
			//加载表格数据
			var $mygrid = jQuery("#listPlanActivity").jqGrid({ 
                url:'/customer/activityInfor.do?method=list&type=2',
                //rownumbers: true,
                datatype: "json",                
               	autowidth: true,               	
				//height: "auto",
				height:300,
                colNames:['Id','公司名称','活动内容','计划时间','附件'],
                colModel:[
                ...
                pager: "#pagerPlanActivity"
                //caption: "已收到的讯息"
        }).navGrid('#pagerPlanActivity',{edit:false,add:false,del:false,search:false});     
        
        $("gbox_listPlanActivity").css("width","100%");  
      		
		//自定义按钮
		$("#listPlanActivity").jqGrid('navButtonAdd','#pagerPlanActivity', {
			caption:"<span style='color: red;'>批量删除</span>", title:"点击批量删除", buttonicon:'ui-icon-closethick', onClickButton: deleteActivity
		});
		
		$("#listPlanActivity").jqGrid('navButtonAdd','#pagerPlanActivity', {
			caption:"查询", title:"点击查询计划中活动信息", buttonicon:'ui-icon-search', onClickButton: openMultipleSearchDialog
		});
		
		//批量删除
		function deleteActivity(){
			doDelete("/customer/activityInfor.do?method=delete","listPlanActivity");
		}
