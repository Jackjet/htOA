...
function initHead(a)
{
	...
	{
		var obj = document.getElementById(colArray[i]);
		alert(colArray[i]);
		alert(obj.childNodes.length);
		for(var j=0;j<obj.childNodes.length-1;j++)
		{
			var module_i=obj.childNodes[j];
...
function hide_msg(msgBlock,flag)
{
   $(msgBlock).innerHTML = msgBlockHTML;
   if(flag==1) $("overlay").style.display = 'none';
...
function CorrectImg()
{
   ...
   for(var i=0; i<imgs.length; i++)
   {
      ...
      var padding_top=Math.ceil((20-el.height)/2)-1;
