...
<script>
	...
	<%--function editCheckInfor(checkId) {--%>
		<%--var path = "<c:url value='/gcpurchase/checkInfor.do'/>?method=edit&chargerEdit=true&instanceId=${_Purchase.instanceId}&checkId="+checkId;--%>
		<%--window.name = "__self";--%>
...
<link href="<c:url value="/"/>css/submit.css" type="text/css" rel="stylesheet">
	...
	<div>
	<c:if test="${Purchase.flowId.flowId == 3 }">
		...
		<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-btable" style="width: 90%">
			<tbody id="tblGrid">	
						<c:if test="${_Purchase.flowId.flowId == 3}">
							<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
									<td class="ui-state-default jqgrid-rownum" style="width: 20%">项目名称：</td>
									<td>${_Purchase.purchaseTitle}</td>
							</tr>
							<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
								<td class="ui-state-default jqgrid-rownum" style="width: 20%">需求：</td>
								<td>${_Purchase.purchaseGoods}</td>
							</tr>
							<c:if test="${_Purchase.purchaseNumber != 0}">
							<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
								<td class="ui-state-default jqgrid-rownum" style="width: 20%">预计工期：</td>
								<td>${_Purchase.purchaseNumber}天</td>
							</tr>
							</c:if>
							<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
								<td class="ui-state-default jqgrid-rownum" style="width: 20%">工程类型：</td>
								<td>${_Purchase.guige}</td>
							</tr>
							<c:if test="${_Purchase.gcfa != null}">
							<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
								<td class="ui-state-default jqgrid-rownum" style="width: 20%">工程方案：</td>
								<td>${_Purchase.gcfa}</td>
							</tr>
							</c:if>
							<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
