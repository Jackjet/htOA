...
[#-- 注:使用自定义标签时,controlName要放在最后 --]
						...
						<td><script>
								function createDocumentNo() {
									...
									if (organizeId != '' && organizeId.length != null) {
										...
										var serialNo = "";
										$.ajax({
											url: "/workflow/submit.do?method=getSerialNo&tableName=Customize_fawenguanli&fieldName=serialNo&fieldYear=reportYear&reportYear="+sysYear,
											type: "post",
											dataType: "json",
											async: false,	//设置为同步
										...
										var shortName = "";
										$.ajax({
											url: "/core/organizeInfor.do?method=getShortName&departmentId="+organizeId,
											type: "post",
											dataType: "json",
											async: false,	//设置为同步
											beforeSend: function (xhr) {
											...
											complete : function (req, err) {
												var returnValues = eval("("+req.responseText+")");
												shortName = returnValues["_ShortName"];
											}
										});
										//documentNos[0].value = "沪港物流"+shortName+"发["+sysYear+"]第 号";
										documentNos[0].value = "HT-FW-"+sysYear+"-"+serialNo;
								...
								function loadSelUsers(){
									var organizeId = $("#organizeId option:selected").val();
									//alert(organizeId);
						            var url = "/core/systemUserInfor.do?method=getUsers&departmentId=" + organizeId;
						            $("#writerId").selectInit();
						            $.loadUsers(url, "writerId");
						            $("#reviewerId").selectInit();   
						            $.loadUsers(url, "reviewerId");   
								}
								if ('${_InstanceId}'==null||'${_InstanceId}'=='') {
									$("#organizeId").change(function(){createDocumentNo();loadSelUsers();});
								}
						</script></td>
					</tr>
					<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
					...
					<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
						...
						<td colspan="3">[@input size="50" controlName="sendPersons"/]</td>
					</tr>
					<tr class="ui-widget-content jqgrow ui-row-ltr" style="height: 30px;">
						<td class="ui-state-default jqgrid-rownum">抄送：</td>
						<td colspan="3">[@input size="50" controlName="copyPersons"/]</td>
					</tr>
					<tr style="display: none;">
						<td>[@searchControl searchType="input" searchField="documentNo" searchOper="cn" searchName="流水号"/]</td>
                        <td>[@searchControl searchType="input" searchField="indexNo" searchOper="cn" searchName="发文号"/]</td>
